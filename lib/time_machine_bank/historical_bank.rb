...
module TimeMachineBank
  ...
  class HistoricalBank < Money::Bank::Base
    include TimeMachineBank::OpenExchangeRatesLoader
    extend TimeMachineBank::HistoricalBankConfigure
    ...
    def get_rate(date, from, to)
      rate = TimeMachineBank::HistoricalBank.load(TimeMachineBank::HistoricalBank.build_key(date, from, to))
    ...
    # @return [Money]
    #
    # @raise +TimeMachineBank::UnknownRate+ if the conversion rate is unknown.
    #
    # @example
    #   bank = TimeMachineBank::VariableExchange.new
    ...
    # @return [String]
    #
    # @raise +TimeMachineBank::UnknownRateFormat+ if format is unknown.
    #
    # @example
    #   bank = TimeMachineBank::VariableExchange.new
    ...
    def export_rates(format, file=nil)
      raise TimeMachineBank::UnknownRateFormat unless
    ...
    def rate_key_for(from, to)
      "#{Money::Currency.wrap(from).iso_code}_TO_#{Money::Currency.wrap(to).iso_code}".upcase
    end
