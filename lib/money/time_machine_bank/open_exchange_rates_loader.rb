...
module TimeMachineBank
  module OpenExchangeRatesLoader
    ...
    def load_data(date)
      date = date.to_date
      date_formated = date.strftime(TimeMachineBank::HistoricalBank.config.date_format)
      ...
      doc['rates'].each do |currency, rate|
        ...
        TimeMachineBank::HistoricalBank.store(TimeMachineBank::HistoricalBank.build_key(date, base_currency, currency), rate)
      end
    end
  end
end